        ...
        <div className="bg-[linear-gradient(180deg,#04121f_0%,#082033_30%,#f2fbff_30%,#f2fbff_100%)]">
            <section className="width-primary mx-auto px-4 pb-16 pt-12 text-white lg:pt-16">
                <div className="max-w-4xl space-y-4">
                    <div className="text-xs font-semibold uppercase tracking-[0.22em] text-bela-primary-3">Chi tiết sản phẩm</div>
            ...
            <section className="width-primary mx-auto px-4 pb-20">
                <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
                    ...
                    <aside className="rounded-[2rem] border border-bela-gray-2 bg-white p-6 shadow-sm md:p-8">
                        <div className="text-xs font-semibold uppercase tracking-[0.18em] text-bela-primary-1">Giá</div>
                        <div className="mt-3 text-3xl font-bold text-bela-secondary-2">
                            {product.price ? `${Number(product.price).toLocaleString('vi-VN')}đ` : 'Liên hệ'}
                        </div>
                        {product.unit ? <div className="mt-1 text-sm text-bela-neutral-3">Đơn vị: {product.unit}</div> : null}
                        <Link href={GUEST_STORE} className="mt-6 inline-flex rounded-xl bg-gradient-to-r from-bela-primary-1 to-bela-primary-2 px-5 py-3 text-sm font-semibold text-white shadow-bela-primary transition hover:-translate-y-0.5">
                            ← Quay lại sản phẩm
                        </Link>
                    </aside>
                </div>
